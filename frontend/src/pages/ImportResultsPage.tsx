...
import { Download, Copy, Search, CheckCircle, XCircle, ChevronLeft, ChevronRight, FileText, Info, X, ArrowLeft } from 'lucide-react'
...
export function ImportResultsPage() {
  ...
  }, [importId, searchTerm, domainFilter, duplicateFilter, currentPage, pageSize])
  ...
  const fetchUrls = async () => {
    ...
    try {
      ...
      const data = await api.getUrlsByImport(importId, filters, {
        page: currentPage,
        pageSize
      })
  ...
  const fetchAllFilteredUrls = async () => {
    ...
    try {
      const filters = getFilters()
      const allUrls = await api.getAllUrlsByImport(importId, filters)
  ...
  const handleExport = async (format: 'csv' | 'txt') => {
    ...
    try {
      const filters = getFilters()
      const blob = await api.exportUrlsByImport(importId, format, filters)
      const url = window.URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      const extension = format === 'txt' ? 'txt' : 'csv'
      const filename = format === 'txt' ? 'cleaned-urls' : 'urls'
      const importAlias = paginatedData?.urls[0]?.import?.alias || 'import'
      a.download = `${importAlias}-${filename}.${extension}`
  ...
  const importInfo = urls[0]?.import
  const profileInfo = importInfo?.profile
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <div className="flex items-center space-x-2 mb-2">
              <Link
                to={`/profile/${encodeURIComponent(profileInfo?.name || '')}`}
                className="flex items-center space-x-1 text-blue-600 hover:text-blue-800"
              >
                <ArrowLeft className="h-4 w-4" />
                <span>Back to {profileInfo?.name}</span>
              </Link>
            </div>
            <h1 className="text-3xl font-bold text-gray-900">
              Import: "{importInfo?.alias}"
            </h1>
